...
def display_ui(stdscr):
    curses.curs_set(0)
    stdscr.scrollok(True)  # Enables scrolling 
    while True:
        try:
            ...
            lines = [
                ...
                f"👥 Active Addresses: {active_addresses:,}" if active_addresses else "👥 Active Addresses: N/A",
                f"📐 Metcalfe's Ratio: {metcalfe_ratio:.8f}" if metcalfe_ratio else "📐 Metcalfe's Ratio: N/A",
            ...
            # Appends new output so it doesn't clear the screen
            for line in lines:
                stdscr.addstr(line + "\n")
            stdscr.refresh()
            time.sleep(60)
